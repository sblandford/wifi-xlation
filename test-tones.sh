...
start_all () {
  # Audio/Video channel
  ffmpeg -re -f lavfi -i "sine=frequency=218:sample_rate=48000" -f lavfi -i "testsrc=size=1280x720:rate=30" -c:v libvpx -vf format=yuv420p -b:v 200k -an -f rtp -payload_type 98 rtp://$IP:5008 -c:a libopus -b:a 32k -vn -f rtp -payload_type 96 rtp://$IP:5006 &>/dev/null &
  # Audio channels
